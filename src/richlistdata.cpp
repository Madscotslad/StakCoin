		      ...
		      if(!fInvalid)
		      {
			    ...
			    { 
			        ...
			        if(ExtractDestination(atxout.scriptPubKey, dest))
			        {
			    	    ...
			    	    std::string STAKAddress = CBitcoinAddress(dest).ToString();
				        CRichListDataItem richListDataItem;
				        richListDataItem.dAddress = STAKAddress;
				        if(richListData.mapRichList.find(STAKAddress) == richListData.mapRichList.end())
				            richListData.mapRichList.insert(make_pair(STAKAddress, richListDataItem));
					
				        richListData.mapRichList[STAKAddress].nBalance = richListData.mapRichList[STAKAddress].nBalance - atxout.nValue;

pchMessageStart// Copyright (c) 2009-2010 Satoshi Nakamoto
...
int64_t GetProofOfWorkReward(int nHeight, int64_t nFees)
{
	...
	if (pindexBest->nHeight == 1)
        ...
        nSubsidy = 1 * COIN/2;	
	else if (pindexBest->nHeight <= 1000000) // Block 1m ~ 3m STAK (33% will go to hybrid masternodes)
		nSubsidy = 3 * COIN;	
	else if (pindexBest->nHeight <= 2000000) // Block 2m ~ 4m STAK
		nSubsidy = 4 * COIN;
	else if (pindexBest->nHeight <= 3000000) // Block 3m ~ 3m STAK
		nSubsidy = 3 * COIN;		
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    */
    ...
    // ----------- masternode payments -----------
    // Once upon a time, People were really interested in STAK.
    // So much so, People wanted to bring STAK to the moon. Even Mars, Sooner than the roadster...
    ...
    // People wanted masternodes... Really Bad. But King Carsen was already busy with the rest of STAK
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "Blockcoin goes on the market today hitting zero pence";
        CTransaction txNew;
        txNew.nTime = 1521651337;
        txNew.vin.resize(1);
        txNew.vout.resize(1);

    ...
    model(0)
{
    ...
    ui->STAKAvailableLabel->setTextFormat(Qt::RichText);
    ...
    ui->CryptopiaSTAKLabel->setTextFormat(Qt::RichText);
...
//    connect(ui->TradingTabWidget, SIGNAL(tabBarClicked(int)), this, SLOT(on_TradingTabWidget_tabBarClicked(int)));
    ...
    CreateOrderBookTables(*ui->BidsTable,QStringList() << "SUM(BTC)" << "TOTAL(BTC)" << "STAK(SIZE)" << "BID(BTC)");
    CreateOrderBookTables(*ui->AsksTable,QStringList() << "SUM(BTC)" << "TOTAL(BTC)" << "STAK(SIZE)" << "ASK(BTC)");
    /*OrderBook Table Init*/

    /*Market History Table Init*/
    ui->MarketHistoryTable->setColumnCount(5);
    ui->MarketHistoryTable->verticalHeader()->setVisible(false);
    ui->MarketHistoryTable->setHorizontalHeaderLabels(QStringList()<<"DATE"<<"BUY/SELL"<<"BID/ASK"<<"TOTAL DENARIUS(STAK)"<<"TOTAL COST(BTC)");
...
void tradingDialog::UpdaterFunction(){
    //STAKst get the main exchange info in order to populate qLabels in maindialog. then get data
...
QString tradingDialog::GetMarketSummary(){
     ...
     QString Response = sendRequest("https://www.cryptopia.co.nz/api/GetMarket/STAK_BTC");
     return Response;
}

QString tradingDialog::GetOrderBook(){

      QString  Response = sendRequest("https://www.cryptopia.co.nz/api/GetMarketOrders/STAK_BTC/10000");
      return Response;
}

QString tradingDialog::GetMarketHistory(){
    QString Response = sendRequest(QString("https://www.cryptopia.co.nz/api/GetMarketHistory/STAK_BTC/100"));
    return Response;
}
...
QString tradingDialog::BuySTAK(QString OrderType, double Quantity, double Rate){

    QString str = "";
    QString URL = "https://www.cryptopia.co.nz/api/SubmitTrade";
            /*URL += OrderType;
            URL += "/api/GetMarket/STAK_BTC";
            URL += this->ApiKey;
            URL += "&nonce=12345434&market=BTC-STAK&quantity=";
            URL += str.number(Quantity,'i',8);
            URL += "&rate=";
            URL += str.number(Rate,'i',8);*/
    QJsonObject stats_obj;
    stats_obj["Market"] = "STAK/BTC";
    stats_obj["Type"] = "Buy";
...
QString tradingDialog::SellSTAK(QString OrderType, double Quantity, double Rate){
...
QString tradingDialog::GetDepositAddress(){
    ...
    QString Response = sendRequest(URL, "POST", QString("{\"Currency\":\"STAK\"}"));
    return Response;
}
...
int tradingDialog::SetExchangeInfoTextLabels(){
    ...
    ui->volumet->setText("<b>STAK Volume:</b> <span style='font-weight:100; font-size:19px; color:#ffeb55;'>" + str.number(obj["Volume"].toDouble(),'i',8) + "</span> STAK");
...
void tradingDialog::ParseAndPopulateOrderBookTables(QString OrderBook){
    ...
    ui->STAKSupply->setText("<b>Supply:</b> <span style='font-weight:bold; font-size:12px; color:#ffeb55'>" + str.number(STAKSupply,'i',8) + "</span><b> STAK</b>");
    ...
    ui->STAKDemand->setText("<b>Demand:</b> <span style='font-weight:bold; font-size:12px; color:#ffeb55'>" + str.number(STAKDemand,'i',8) + "</span><b> STAK</b>");
...
void tradingDialog::ActionsOnSwitch(int index = -1){
    ...
    switch (index){
                case 0:    //buy tab is active
                    ...
                    Response2 = GetBalance("STAK");
                    Response3 = GetOrderBook();

                    if((Response.size() > 0 && Response != "Error") && (Response2.size() > 0 && Response2 != "Error")){
                        DisplayBalance(*ui->BtcAvailableLabel, *ui->STAKAvailableLabel, Response, Response2);
                    }
                ...
                case 1: //Cross send tab active
                    Response = GetBalance("STAK");
                    Response2 = GetBalance("BTC");
                    if((Response.size() > 0 && Response != "Error") && (Response2.size() > 0 && Response2 != "Error")){
                        DisplayBalance(*ui->CryptopiaSTAKLabel, *ui->CryptopiaBTCLabel, Response, Response2);
                ...
                case 5://show balance tab
                    ...
                    Response = GetBalance("STAK");
                    if(Response.size() > 0 && Response != "Error"){
                        DisplayBalance(*ui->STAKBalanceLabel,*ui->STAKAvailableLabel_2,*ui->STAKPendingLabel, QString::fromUtf8("STAK"),Response);
                    }
                break;
...
void tradingDialog::CalculateSellCostLabel(){
    ...
    double Quantity = ui->UnitsInputSTAK->text().toDouble();
    double cost = ((price * Quantity) - ((price * Quantity / 100) * 0.2));
...
void tradingDialog::CalculateCSReceiveLabel(){
    ...
    QString balance = GetBalance("STAK");
    ...
    double AvailableSTAK = BalanceObject["Available"].toDouble();
...
{
    //calculate amount of BTC that can be gained from selling STAK available balance
    QString responseA = GetBalance("STAK");
    QString str;
    QJsonObject ResultObject =  GetResultObjectFromJSONArray(responseA);

    double AvailableSTAK = ResultObject["Available"].toDouble();

    ui->UnitsInputSTAK->setText(str.number(AvailableSTAK,'i',8));
}
...
{
    double Quantity = ui->CryptopiaSTAKLabel->text().toDouble();
    double Received = 0;
    double Qty = 0;
    double Price = 0;
    QString buyorders = GetOrderBook();
...
void tradingDialog::on_Withdraw_Max_Amount_clicked()
{
    ...
    ui->WithdrawUnitsInput->setText(str.number(AvailableSTAK,'i',8));
}
...
void tradingDialog::on_BuySTAK_clicked()
{
    double Rate;
    double Quantity;
    ...
    if (reply == QMessageBox::Yes) {
        ...
        QString Response =  BuySTAK(Order,Quantity,Rate);
        QJsonDocument jsonResponse = QJsonDocument::fromJson(Response.toUtf8());          //get json from str.
        QJsonObject  ResponseObject = jsonResponse.object();                              //get json obj
...
void tradingDialog::on_SellSTAKBTN_clicked()
{
    double Rate;
    double Quantity;

    Rate     = ui->SellBidPriceEdit->text().toDouble();
    Quantity = ui->UnitsInputSTAK->text().toDouble();
    ...
    if (reply == QMessageBox::Yes) {
        ...
        QString Response =  SellSTAK(Order,Quantity,Rate);
        QJsonDocument jsonResponse = QJsonDocument::fromJson(Response.toUtf8());          //get json from str.
        QJsonObject  ResponseObject = jsonResponse.object();                              //get json obj
    ...
    if(!ctx.isValid())
    {
        ...
        foreach (const QJsonValue & value, BuyArray)
        {
            ...
            if ( ((Quantity / x) - y) > 0 )
            {
                ...
                QString SellResponse = SellSTAK(Order,y,x);
                ...
                QString SellResponse = SellSTAK(Order,(Quantity / x),x);
                QJsonDocument SelljsonResponse = QJsonDocument::fromJson(SellResponse.toUtf8());          //get json from str.
                QJsonObject SellResponseObject = SelljsonResponse.object();                              //get json obj
                ...
                if (SellResponseObject["Success"].toBool() == false){
                    ...
                    if (ResponseObject["Success"].toBool() == false){
                        ...
                        if (ResponseObject["Success"].toBool() == false){
                            ...
                            QMessageBox::information(this,"Success","<center>Cross-Send Successful</center>\n Sold "+Astr.number(Qty,'i',4)+" STAK for "+Qstr.number((ui->CSUnitsInput->text().toDouble()-0.0002),'i',8)+" BTC");
                        }
                    } else if (ResponseObject["Success"].toBool() == true){
                        QMessageBox::information(this,"Success","<center>Cross-Send Successful</center>\n Sold "+Astr.number(Qty,'i',4)+" STAK for "+Qstr.number((ui->CSUnitsInput->text().toDouble()-0.0002),'i',8)+" BTC");
                    }
...
void tradingDialog::on_UnitsInputSTAK_textChanged(const QString &arg1)
{
     CalculateSellCostLabel(); //update cost
}
